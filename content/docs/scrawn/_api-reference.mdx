...
Manually track AI token usage from an `onFinish` or `onStepFinish` callback:

```typescript
await biller.trackAI({ userId, event, inputDebit, outputDebit, inputCacheDebit, outputCacheDebit });
```
