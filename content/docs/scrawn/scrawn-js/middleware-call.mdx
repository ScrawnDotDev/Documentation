...
- **Description:** Function to extract userId and debit info from the request. Return `null` to skip tracking.
- **Returns:** [EventPayload](/docs/scrawn/api-reference#eventpayload) object with `userId` (string) and either `debitAmount` (number) or `debitTag` (string), or `null`
