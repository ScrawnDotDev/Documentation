...
- **Description:** Function to extract userId, debitAmount, and optionally tags from the request. Return `null` to skip tracking.
- **Returns:** [EventPayload](/docs/scrawn/api-reference#eventpayload) object with `userId` (string), `debitAmount` (number), and optional `tags` (string[]), or `null`
...
app.use(scrawn.middlewareEventConsumer({
  extractor: (req): EventPayload => {
    ...
    if (endpoint.startsWith('/api/ai')) {
      debitAmount = 100;
      tags = ['api-call', 'ai-generation'];
    } else if (endpoint.startsWith('/api/premium')) {
      debitAmount = 50;
      tags = ['api-call', 'premium'];
    } else if (endpoint.startsWith('/api/standard')) {
      debitAmount = 10;
      tags = ['api-call', 'standard'];
    }
