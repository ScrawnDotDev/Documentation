...
The middleware intercepts HTTP requests, extracts userId and  debit info (debitAmount or debitTag) via your custom extractor function, and automatically tracks the usage event. It supports whitelist/blacklist patterns with wildcards for fine-grained control.
...
**In simple terms:** When a request comes in, grab the `userId` from the request headers and the debit info (`debitAmount` or `debitTag`) from the request, then charge that user.
</Callout>
...
- **Description:** Function to extract userId and debit info (debitAmount or debitTag) from the request. Return `null` to skip tracking.
- **Returns:** [EventPayload](/api-reference#eventpayload) object with `userId` (string) and either (`debitAmount` (number) or `debitTag` (string)), or `null`
...
<Cards>
  <Card href="/scrawn-js/advanced-usage" title="Advanced Usage">
    Some advanced usage patterns
  </Card>
  <Card href="/scrawn-js/framework-integration" title="Framework Integration">
