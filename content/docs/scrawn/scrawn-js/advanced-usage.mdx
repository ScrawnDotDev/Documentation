...
const events = [
  { userId: 'user-1', debitAmount: 100, tags: ['api-call'] },
  { userId: 'user-2', debitAmount: 200, tags: ['api-call'] },
  { userId: 'user-3', debitAmount: 150, tags: ['api-call'] },
];
...
usageQueue.process(async (job) => {
  const { userId, debitAmount, tags } = job.data;
  await scrawn.sdkCallEventConsumer({ userId, debitAmount, tags });
});
```
