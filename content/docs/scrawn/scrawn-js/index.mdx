...
// Track a billable event with a direct amount
await scrawn.sdkCallEventConsumer({
  userId: 'user-123',
  debitAmount: 100,
});

// Track a billable event using a backend price tag
await scrawn.sdkCallEventConsumer({
  userId: 'user-123',
...
Track usage events programmatically for billing purposes. This is a general-purpose event consumer that can be abstracted for various use cases. [View Details](/scrawn-js/sdk-call-event#parameters).

### `middlewareEventConsumer()`
Automatically track API requests with Express-compatible middleware. Supports whitelist/blacklist patterns with wildcards. [View Details](/scrawn-js/middleware-call#configuration).

### `collectPayment()`
Generate a checkout link to collect payment from users. [View Details](/scrawn-js/collect-payment#parameters).
