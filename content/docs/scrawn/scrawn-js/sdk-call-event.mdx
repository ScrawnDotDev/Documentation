...
### `tags` (optional)
- **Type:** `string[]`
- **Description:** Array of string tags to categorize and label the event. Use tags to organize events for analytics, filtering, and reporting purposes.
- **Validation:** Must be an array of strings (if provided)
- **Example:** `["api-call", "premium"]`, `["ai-generation", "gpt-4"]`, `["storage", "upload"]`
...
async function trackTokenUsage(userId: string, tokensUsed: number) {
  ...
  await scrawn.sdkCallEventConsumer({
    ...
    tags: ['ai-generation', 'tokens', 'gpt-4'],
  });
}
...
Tags help you organize and analyze your usage events. Here are some best practices:
...
await scrawn.sdkCallEventConsumer({
  userId: 'user-123',
  debitAmount: 5.00,
  tags: ['image-generation', 'ai', 'premium-tier'],
});
...
await scrawn.sdkCallEventConsumer({
  userId: 'user-456',
  debitAmount: 0.50,
  tags: ['api-call', 'analytics', 'v2'],
});

// Combine tags for detailed categorization
await scrawn.sdkCallEventConsumer({
  userId: 'user-789',
  debitAmount: 2.00,
  tags: ['compute', 'batch-processing', 'high-priority'],
});
```

**Tagging Tips:**
- Use consistent, lowercase tag names
- Be specific but concise (e.g., `gpt-4` instead of `ai-model-gpt-4-turbo`)
- Use tags for filtering and analytics
- Combine multiple tags for fine-grained categorization
- Examples of good tags: `api-call`, `ai-generation`, `storage`, `premium`, `batch-job`
